# How to turn your annotation and audio files into files for submission to the 
#IEN wildlife acoustics dataset project

# Introduction ####

#Through this document, I will guide you through the steps to extract the data from 
#the RAVEN Pro/Lite annotation files as well as the audio files which were annotated 
#into a the finished xlsx file format required for submission for the IEN wildlife 
#acoustics dataset project. This will also make use of personal eBird and/or iNaturalist 
#data to provide the closest coordinates for the recording location of a given audio file. 
#This will require downloading them from the respective websites.
...
# 
# 5.  Go check your associated email ID inbox, and they would have sent an download 
#link there.
# 
# 6.  Click on the url in that email to download the data.
# 
# 7.  Download/Copy the data to a specific folder (same place where the iNaturalist 
#data was downloaded to, if downloaded), and extract the zip file. The zip file will 
#be named something like "ebird_1755536365090.zip", and the unzipped file of interest
#will be "MyEBirdData.csv" in the folder named "ebird_1755536365090".
# 
# Download the annotation template ####
# 
# From the Github project <https://github.com/paulvpop/ecoacoustics>, download the
#file titled "annotation_template.xlsx" which is the cleaned version of the template
#available from the Acoustic Data Upload Form available here:
#https://docs.google.com/forms/d/e/1FAIpQLSctSuX61-9fdV46nuTm_tP4dzOoLL4CoIN2lg7LMI9pHVJb_w/viewform. 
#Keep it in the same folder as the above files.

# RAVEN annotation data ####

#Copy all the annotations files (which will likely be in the folder "Selections" 
#under "RavenPro1.x" where x is the version number. It should be the same in Raven 
#Lite too) to a folder titled "annotations" in the same folder where the iNaturalist 
#and/or eBird data and the annotation template is kept.

# The annotation files provided by RAVEN will be of the following general format 
#(where "Scientific Name" is the column you add).
# 
...
#Copy all the audio files that were used for annotation to the same folder as above 
#and name it 'audio'
...
#Open RStudio (first install R and RStudio it if you haven't, using the help from here: 
#https://posit.co/download/rstudio-desktop/
                                                                                          
## Step 1. Set the working directory. ####
                                                                                        
#This will be the folder containing all the above files. For this, Press Ctrl+Shift+H 
#to interactively choose the wanted directory OR (for Windows) copy (but don't paste) 
#the directory path, which will look like this: "D:/Name/Bioacoustics" and run the next two lines of code

FolderPath <- normalizePath(readClipboard(), "/")
setwd(FolderPath)

#In Linux, one can just copy-paste the folderpath (can be found in the filepath pane or 
#if the folder is opened in the terminal) directly. Don't forget to change the 
#name of the path in quotes with your own:
...
#To do this, first install the package readxl if you don't have it already, and 
#then load this library.

install.packages("readxl")
library(readxl)

#Then we will load the relevant files one by one. The str() functions shows all the 
#columns within those files.
...
#Of interest in the iNat data is the column "observed_on_string" which gives the 
#date and time as a string in IST format (which is important since the dataset is 
#India-focused). Check the first 10 observations in the column to see how it looks like:
...
#[1] "2025-07-31 11:36:37" "2025-07-31 12:52:33" "2025-07-31 15:14:03" "2025-07-31 15:24:00" 
#[5] "2025-07-31 15:25:37" "2025-08-03 09:31:38" "2025-08-15 17:32:33" "2025-08-15 17:45:41" 
#[9] "2025-08-15 18:11:41" "2025-08-16 16:44:58"

#We see that iNaturalist date-time is not standardised. So, it needs to be done in 
#a later step.

#Of interest in the eBird data are the columns "Date" and "Time" which gives the 
#date and time as strings in IST format. Check the first 10 observations to see how it looks like:

head(ebird$Date, 10)
#[1] "2024-01-15" "2024-01-15" "2024-01-15" "2025-02-16" "2024-12-29" "2025-04-20" 
#[7] "2024-07-13" "2024-10-01" "2024-08-11" "2024-07-13"

head(ebird$Time, 10)
#[1] "08:00 AM" "07:12 AM" "06:40 AM" "11:50 AM" "06:00 PM" "05:41 AM" "08:11 AM" 
#[8] "07:07 AM" "09:07 AM" "08:30 AM"

#We know that eBird Date and Time are standardised as it is outputted as such 
#(in IST in case of this dataset).
...
#The LineCount column is important as it tells how many annotations (bounding boxes) 
#you have made. Check the total number of annotations you have made (Subtracting the 
#rows containing the column headers using no. of rows in 'data' as proxy):
...
#Rename the column names in the 'combined_annotations' object, add the missing 
#columns (from the 'Annotations' object), and rearrange the columns to the match 
...
#So, there are 27 classes and 25 species (Vehicular noise and Recorder movement 
#noise excluded) in all my annotations put together
...
# First, convert data_audio's 'FileModifyDate' column to proper datetime, after 
#removing the timezone offset (+05:30)
...
#   
# Pattern 1: The explanation for the above code is that grepl function is used to
# check if a string matches pattern: YYYY/MM/DD HH:MM AM/PM. The components of the 
# string: \^ = start of string, \\d{4} = check if its exactly 4 digits (year), / = 
#   literal slash, \\d{2} = check if its exactly 2 digits (for month, day, minute), 
# \\d{1,2} = check if its either 1 or 2 digits (hour), : = literal colon, [AP]M =
#   check if its either "AM" or "PM" parse_date_time function tries to parse using 
# format "Year/month/day Hour:Minute AM/PM" If that fails, it tries "Year/month/day 
# Hour:Minute" (without AM/PM)
# 
# Pattern 2: The grepl function checks if string matches pattern: YYYY-MM-DD HH:MM:SS
# Code explanation similar to Pattern 1, but in this case instead of literal slashes, 
# we check matches for hyphens, and there is an addition of second information, and 
# there are no AM/PM classification as it is a 24 hour format. ymd_hms converts the 
# string to datetime object
# 
# Pattern 3: The grepl functions check if string matches pattern: YYYY/MM/DD. ymd 
# converts the string to date object (time set to 00:00:00)
# 
# Pattern 4: The grepl function checks if the string matches pattern: YYYY-MM-DD
# 
# Pattern 5: The grepl function checks if the string matches pattern: YYYY-MM-DD Noon.
# Then deletes 'Noon'.
# 
...
#Note that the above for-loop will also add a column titled 'Data Source' which 
#will either say eBird or iNaturalist. It means that the latitude and longitude 
#data is coming from one of these sources. This is for you to cross-reference 
#some coordinates or check if there are any errors. You should ideally delete this 
#column before submission. You can use the following for this step:
...
#The only mandatory column in Meta not filled is the "Authors" column as different 
#recordings may have multiple authors. You can fill it up (with commas separating 
#the authors if multi-author) once you download the file at the end of this walkthrough.


#h Rename the 'Media File Name/Media URL' rows in Meta to match that of 
'Media File Name/Media URL'

#This step is necessary since the required Media File Name/Media Url is not
#the name of the annotations file, but the audio file name.

# To do this, first extract the base filename from Annotations by removing the extensions
# such as 'annotations.txt', '.txt' or 'Table.1.selections.txt'
#First install and load stringr package if already not installed:
if (!require("stringr")) install.packages("stringr") 
library(stringr)

Annotations <- Annotations %>%
  mutate(base_filename = str_remove(`Media File Name/Media URL`, 
                                    "\\.(annotations\\.txt|Table\\.1\\.selections\\.txt|txt)$"))

# Create a matching base filename in Meta by removing audio file extensions
Meta <- Meta %>%
  mutate(base_filename = str_remove(`Media File Name/Media URL`, 
                                    "\\.(wav|mp3|flac|WAV|MP3|FLAC)$"))

# Join using the base filenames (add the 'Media File Name/Media URL' column from Meta
# to Annotations by matching the base filenames present in Meta_with_base and 
# Annotations)
Annotations <- Annotations %>%
  left_join(Meta %>% select(base_filename, `Media File Name/Media URL`), 
            by = "base_filename",
            suffix = c("_old", ""))

# Check for unmatched files and throw warnings
unmatched_files <- Annotations %>%
  filter(is.na(`Media File Name/Media URL`)) %>%
  distinct(base_filename)

if(nrow(unmatched_files) > 0) {
  warning("The following files from Annotations did not find matches in Meta:\n",
          paste("- ", unmatched_files$base_filename, collapse = "\n"))
}

# For the 'Media File Name/Media Url' column in Annotations, use the matched 
# filenames from Meta, and (if there are any such cases) keep original for the unmatched ones
Annotations <- Annotations %>%
  mutate(`Media File Name/Media URL` = ifelse(
    is.na(`Media File Name/Media URL`),
    `Media File Name/Media URL_old`,
    `Media File Name/Media URL`
  )) %>%
  select(-base_filename, -`Media File Name/Media URL_old`)

# The 'Media File Name/Media URL' column as at the end. It has to be brought to the front:
Annotations <- Annotations %>% select(`Media File Name/Media URL`, everything())

#Remove the intermediary basefile name in Meta:
Meta <- Meta %>% select(-base_filename)

# Data input ####
#Now that we have the two main sheets, which have data extracted from multiple 
#sources, we can fill up the remaining sheet, i.e. "Authors" sheet. Replace the 
#content in the double quotes with your data in the following code chunk:
Authors <- data.frame(
...
#Now that we have all the files ready, combine all the files 'Meta', 'Annotations',
#and 'Authors':
...
#Change the working directory again back to the main folder ("\~/Wildlife/Bioacoustics/For submission") 
#in this example. Then save the file there.
...
# Create a directory (if it has not been created already). NOTE: Change the folder 
# name to match with the naming of the annotation xlsx in your case.
...
# You now have a zip file containing the folder to be submitted. Submit the zip 
# file using this Acoustic Data Upload Form: 
# https://docs.google.com/forms/d/e/1FAIpQLSctSuX61-9fdV46nuTm_tP4dzOoLL4CoIN2lg7LMI9pHVJb_w/viewform.
